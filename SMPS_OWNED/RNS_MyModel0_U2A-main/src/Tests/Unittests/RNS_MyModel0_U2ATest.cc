...
// CHECKSUM:c740bb82971d3d1608ac4bcb8d372a0fad1756d0 
// CLASS_ID:testclass_1

#include <RNS_MyModel0_U2ATestHarness.h>
...
class RNS_MyModel0_U2ATest : public RNS_MyModel0_U2ATestHarness {

  // Register the test class and all test methods
  SCML2_BEGIN_TESTS(RNS_MyModel0_U2ATest);
  SCML2_TEST(testAlwaysSucceeds);
  SCML2_END_TESTS();

  RNS_MyModel0_U2ATest() {
  }
  ...
  //virtual RNS_MyModel0_U2A* create_model_under_test(const char* name) { // ! Only available when not reusing component build in test
  //  return new RNS_MyModel0_U2A(name); // ! Only available when not reusing component build in test
  //}

  // Override this if you want to customize construction of your mirror model
  //virtual RNS_MyModel0_U2AMirrorBase* create_mirror_model(const char* name) {
  //  return new RNS_MyModel0_U2AMirrorBase(name);
  //}
...
//SCML2_REGISTER_TEST_GROUP_WITH_PARAMETERS(RNS_MyModel0_U2ATest, parameter_configurations(parameter_configuration("Default")));
SCML2_REGISTER_TEST_GROUP(RNS_MyModel0_U2ATest);
